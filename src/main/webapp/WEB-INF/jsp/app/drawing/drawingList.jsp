...
<div style="padding-bottom:5px;padding-left: 0px;">
    ...
    <div id="form1">
        ...
        <table>
            ...
            <td>
                <input emptyText="请输入开始时间" allowInput="false" style="width: 200px;" name="startDateTime"
                       class="mini-datepicker" format="yyyy-MM-dd HH:mm:ss" timeFormat="HH:mm:ss" showTime="true"
            ...
            <td>
                <input emptyText="请输入结束时间" allowInput="false" style="width: 200px;" name="endDateTime"
                       class="mini-datepicker" format="yyyy-MM-dd HH:mm:ss" timeFormat="HH:mm:ss" showTime="true"
                       showOkButton="true" showClearButton="false"/>
            </td>
            <td align="center">
...
<div style="width:99%;">
    <div class="mini-toolbar" style="border-bottom:0;padding:0px;">
        <table style="width:100%;">
            <tr>
                <td style="width:100%;">
                    <form action="/drawing/exportDrawing.action" method="post" id="form2">
                        <input type="hidden" name="queryParam" id="queryParam" class="mini-hidden">
                        <a class="mini-button" iconCls="icon-add" onclick="add()">提款出金</a>
                        <a class="mini-button" iconCls="icon-download" onclick="exportDrawing()">导出</a>
                    </form>
                </td>
            </tr>
        </table>
...
<div id="datagrid1" class="mini-datagrid" style="width:99%;height:69%;" sizeList="[20,30,50,100]" pageSize="20"
     url="/drawing/queryDrawing.action" idField="id" allowResize="true" allowCellWrap="true"
...
<script type="text/javascript">
    ...
    function onActionRenderer(e) {
        ...
        var s;
        if (record.drawingApplyStatusEnum.code == 0) {
            s = ' <a class="Edit_Button" href="javascript:audit(\''
                    + record.id + '\')">审核</a>';

        } else if (record.drawingApplyStatusEnum.code == 1) {
            s = '通过';
        } else {
            s = '不通过';
        }
    ...
    function add() {
        mini.open({
            url: "<c:url value='/drawing/addDrawingPage.action'/>",
            title: "提款出金", width: 300, height: 300,
            onload: function () {
    ...
    function audit(e) {
        mini.open({
            url: "<c:url value='/drawing/auditDrawingPage.action'/>",
            title: "审核", width: 350, height: 250,
            onload: function () {
                var iframe = this.getIFrameEl();
                var data = {id: e};
